...
- [x] T009 [US1] Validate User Story 1 and record checkpoint evidence in `F:/github/launchdeck/.specify/features/2026-07-07-launchdeck-v1-cross/implementation-review/reviews.ndjson`

### Repair Window for User Story 1

- [x] T023 [US1] [repair_for: T004,T009] Add regression tests for JSON-mode foreground stdout isolation and config-resolved failure context in `F:/github/launchdeck/test/cli-contract.test.js`
- [x] T024 [US1] [repair_for: T008,T009] Fix foreground run JSON stdout isolation and config-resolved `task_not_found` context in `F:/github/launchdeck/src/cli.js` and `F:/github/launchdeck/src/runtime.js`
- [x] T025 [US2] [repair_for: T015] Update legacy CLI smoke expectations and add config-resolved runtime failure regression coverage in `F:/github/launchdeck/test/cli.test.js` and, if needed, `F:/github/launchdeck/test/managed-cli.test.js`
- [x] T026 [US2] [repair_for: T015] Fix managed CLI failure context and stop partial envelope handling in `F:/github/launchdeck/src/cli.js`
- [x] T027 [US3] [repair_for: T018] Align config tests with runtime-owned clean safety planning in `F:/github/launchdeck/test/config-contract.test.js`
- [x] T028 [US3] [repair_for: T018] Move clean target safety decisions from config normalization to runtime planner input preservation in `F:/github/launchdeck/src/config.js`

**Join Point US1-REPAIR**: Run `node --test test/cli-contract.test.js`; pass condition is the noisy stdout and config-resolved failure context regressions pass, and T009 can be retried.

**Join Point US2-REPAIR**: Run `npm run check` and `npm test`; pass condition is the stale lifecycle smoke is aligned, `ps --json` config-resolved runtime failures include `configPath`, stop partial paths emit a partial envelope, and T015 can be retried.

**Join Point US3-REPAIR**: Run `node --test test/config-contract.test.js test/clean-safety.test.js`; pass condition is clean config raw entries survive loadConfig, runtime planner reports refused targets, no approved target is deleted before a refused-plan failure, and T018 can be retried.
...
- [x] T010 [P] [US2] Add managed runtime state and stale/invalid/log tests in `F:/github/launchdeck/test/runtime-state.test.js`
- [x] T011 [P] [US2] Add platform adapter and managed CLI lifecycle tests in `F:/github/launchdeck/test/managed-cli.test.js`
...
- [x] T012 [US2] Implement process and path adapter modules in `F:/github/launchdeck/src/adapters/process.js` and `F:/github/launchdeck/src/adapters/path.js`
- [x] T013 [US2] Implement runtime state v1, liveness refresh, logs, stop verification, stale/unknown/stop_failed states in `F:/github/launchdeck/src/runtime.js`
- [x] T014 [US2] Implement managed command envelopes, duplicate-start code, restart partial failure, logs, ps, and stop CLI behavior in `F:/github/launchdeck/src/cli.js`
- [x] T015 [US2] Validate User Story 2 and record checkpoint evidence in `F:/github/launchdeck/.specify/features/2026-07-07-launchdeck-v1-cross/implementation-review/reviews.ndjson`
...
- [x] T016 [P] [US3] Add clean dry-run, safe/all confirmation, missing target, root, out-of-root, and symlink/junction safety tests in `F:/github/launchdeck/test/clean-safety.test.js`
...
- [x] T017 [US3] Implement canonical clean target planning and refusal behavior in `F:/github/launchdeck/src/runtime.js` and `F:/github/launchdeck/src/adapters/path.js`
- [x] T018 [US3] Implement clean dry-run/safe/all JSON envelopes and confirmation errors in `F:/github/launchdeck/src/cli.js`
- [x] T019 [US3] Validate User Story 3 and record checkpoint evidence in `F:/github/launchdeck/.specify/features/2026-07-07-launchdeck-v1-cross/implementation-review/reviews.ndjson`
...
- [x] T020 Update v1 user docs, examples, schema, and public error-code references in `F:/github/launchdeck/README.md`, `F:/github/launchdeck/examples/`, and `F:/github/launchdeck/schema/launchdeck.schema.json`
- [x] T021 Add Windows, macOS, and Linux lifecycle smoke workflow in `F:/github/launchdeck/.github/workflows/ci.yml`
- [x] T022 Run full local checks plus quickstart smoke and record verification evidence in `F:/github/launchdeck/.specify/features/2026-07-07-launchdeck-v1-cross/implementation-review/reviews.ndjson`
...
| T015 | quality-reviewer | T012,T013,T014,T026 | false | `implementation-review/reviews.ndjson` | US2 join evidence proves runtime inspectability and stop behavior. | `npm run check; npm test` |
...
| T018 | executor | T016,T017,T028 | false | `src/cli.js` | Clean CLI output and exit behavior match contract. | `node --test test/clean-safety.test.js` |
...
| T023 | test-engineer | T008 | false | `test/cli-contract.test.js` | Regression tests prove `run --json` emits exactly one JSON object even when the foreground task writes stdout, and config-resolved `task_not_found` includes `projectRoot`. | `node --test test/cli-contract.test.js` |
| T024 | executor | T023 | false | `src/cli.js`, `src/runtime.js` | Run JSON mode captures or redirects child stdout outside stdout envelope and config-resolved failures include project context. | `node --test test/cli-contract.test.js` |
| T025 | test-engineer | T014 | false | `test/cli.test.js`, `test/managed-cli.test.js` | Legacy smoke and regression tests reflect current US2 JSON contract and config-resolved runtime failure context. | `node --test test/cli.test.js test/managed-cli.test.js` |
| T026 | executor | T025 | false | `src/cli.js` | `ps --json` runtime failures include config context and stop partial failures emit partial envelopes. | `node --check src/cli.js; npm test` |
| T027 | test-engineer | T017,T018 | false | `test/config-contract.test.js` | Config tests assert clean raw entries are preserved for runtime planning while task cwd/log containment stays strict. | `node --test test/config-contract.test.js test/clean-safety.test.js` |
| T028 | executor | T027 | false | `src/config.js` | Config preserves clean raw entries for runtime planner and clean-safety tests pass with T018 CLI behavior. | `node --check src/config.js; node --test test/config-contract.test.js test/clean-safety.test.js` |
