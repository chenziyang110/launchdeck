...
Write `.specify/discussions/<slug>/handoff-to-specify.json` from `.specify/templates/discussion-handoff-template.json`. The filename is compatibility-only; the payload is one `discussion_requirement_contract` that may select `sp-specify` or `sp-quick`.
