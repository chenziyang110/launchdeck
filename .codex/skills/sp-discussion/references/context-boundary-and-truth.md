...
Preserved Contract: Migrated from `.specify/templates/commands/discussion.md`; this file preserves existing `sp-discussion` behavior and does not define new workflow behavior.
...
4. In that escalation, run `C:\Users\11034\.specify\bin\project-cognition.exe query --query-plan "<query_plan_json>"` and use the returned readiness, route_pack, subgraph, missing coverage, and `minimal_live_reads` only as advisory navigation.
