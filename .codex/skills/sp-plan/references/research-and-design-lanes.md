...
Use `.specify/templates/research-template.md` as the structure when `research.md` is triggered. Prefer official documentation, standards, and primary sources; treat model memory as provisional. Research must reduce planning ambiguity rather than accumulate background reading, and every recommendation names confidence, assumptions, validation, environment/dependency notes, and why hand-rolling is or is not justified.
