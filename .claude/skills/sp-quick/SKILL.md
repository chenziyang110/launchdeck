...
argument-hint: "Describe the bounded quick task, or leave blank to resume the current quick-task workspace"
...
When running `sp-quick` in Claude Code, you are the **leader**, not the concrete implementer.
...
## Claude Code Quick Execution Routing

When running `sp-quick` in Claude Code, do not start execution routing until `STATUS.md` exists and `understanding_confirmed: true` is recorded.
...
- Decision order for Claude Code `sp-quick`: safe packetized subagents -> `managed-team` when durable state is needed -> `subagent-blocked` with reason.
